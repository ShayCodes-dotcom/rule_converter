"""
OIA-to-Drools Rule Converter v3.0.0-G
Flask web application that converts Oracle Intelligent Advisor (OIA) Word rule
documents to Drools DRL files with automatic dependency-aware agenda-group ordering.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# v3: Automatic dependency graph → agenda-group
# ─────────────────────────────────────────────────────────────────────────────

def parse_rule(rule_text: str):
    """Extract fields used (when) and set (then) from generated DRL."""
    clean = re.sub(r'/\*.*?\*/', '', rule_text, flags=re.DOTALL)
    when_match = re.search(r'when\s*(.*?)\s*then', clean, re.DOTALL | re.IGNORECASE)
    then_match = re.search(r'then\s*(.*?)\s*end', clean, re.DOTALL | re.IGNORECASE)

    uses = set()
    sets = set()

    if when_match:
        when_part = when_match.group(1)
        uses.update(re.findall(r'\b([a-zA-Z_]\w*)\b\s*(?:==|!=|>=|<=|>|<)', when_part))
        uses.update(re.findall(r'\$fact\.(\w+)', when_part))
    if then_match:
        then_part = then_match.group(1)
        set_matches = re.findall(r'\$fact\.set([A-Z]\w*)\s*\(', then_part)
        for m in set_matches:
            prop = m[0].lower() + m[1:] if m else m
            sets.add(prop)
    return uses, sets


# ─────────────────────────────────────────────────────────────────────────────
# Utility Functions (from your v2)
# ─────────────────────────────────────────────────────────────────────────────
...
def _is_title(para):
    for r in para.runs:
        if r.font.color and r.font.color.rgb:
            if str(r.font.color.rgb) not in ('000000', '000000'):
                return True
    return False


# ─────────────────────────────────────────────────────────────────────────────
# Your original classes (OIADocumentReader → ConditionTranslator etc.)
# ─────────────────────────────────────────────────────────────────────────────
...
class OIADocumentReader:
    def read(self, filepath):
        ...
        for i, para in enumerate(doc.paragraphs):
            ...
            p_data = {'index': i, 'text': text, 'indent_pt': _get_indent_pt(para), 'is_bold': _is_bold(para), 'is_empty': not text}
        ...
        return {'title': title, 'paragraph_groups': paragraph_groups, 'tables': tables}
    ...
    def _read_table(self, table, table_index):
        ...
        for ri in range(1, len(table.rows)):
            ...
            for p in row.cells[1].paragraphs:
                ...
                if ptxt:
                    pindent = p.paragraph_format.left_indent.pt if p.paragraph_format.left_indent else 0.0
                    cond_paragraphs.append({'text': ptxt, 'indent_pt': pindent})
            is_otherwise = len(cond_paragraphs) == 1 and cond_paragraphs[0]['text'].lower() == 'otherwise'
            rows.append({'value': value_text, 'conditions': cond_paragraphs, 'is_otherwise': is_otherwise})
        return {'table_index': table_index, 'header': header_text, 'rows': rows}


# (All your other classes: ConditionNode, build_condition_tree, ConditionTranslator, ParsedRule,
#  parse_paragraph_rules, parse_table_rules, _analyze_conclusion — copy them exactly from your v2 file
#  and paste them here, replacing the comments below)
...
# Paste your full build_condition_tree, _parse_token_group, _build_mixed_tree, _find_group_end,
# _parse_either_or_group, _parse_any_group, _parse_all_group here
...
class ConditionTranslator:
    ...
    def _register_field(self, field_name, oia_name, field_type):
        if field_name not in self.fields:
            self.fields[field_name] = {'oia_name': oia_name, 'type': field_type, 'notes': []}
        else:
            if self.fields[field_name]['type'] == 'Object' and field_type != 'Object':
...
class ParsedRule:
    def __init__(self):
        ...
        self.conclusion_value = None
        self.conclusion_type = 'boolean'
        self.condition_tree = None
        self.salience = 10
        self.is_otherwise = False
        self.source_type = ''
        self.review_notes = []
        self.oia_conclusion_text = ''
        self.agenda_group = "phase-999"   # ← new for v3

# Paste your full parse_paragraph_rules, parse_table_rules, _analyze_conclusion here
...
def _analyze_conclusion(text):
    ...
    return (field, 'boolean')
# ─────────────────────────────────────────────────────────────────────────────
# DRLGenerator v3.0.0-G (complete)
# ─────────────────────────────────────────────────────────────────────────────
...
class DRLGenerator:
    ...
    def generate(self, paragraph_rules, table_rules, translator, title='', source_file=''):
        all_rules = paragraph_rules + table_rules
        raw_rules = []
        for rule in all_rules:
            raw_text = self._generate_rule(rule, translator)
            raw_rules.append((rule, raw_text))

        self._assign_dependency_ordering(raw_rules)

        lines = self._build_drl_header(source_file, title, translator)

        for rule, raw_text in raw_rules:
            if rule.source_type == 'paragraph':
                lines.append(self._inject_agenda_group(raw_text, rule.agenda_group))

        current_header = None
        for rule, raw_text in raw_rules:
            if rule.source_type == 'table':
        ...
        if translator.review_items:
            lines.append('')
            lines.append('// ────────────────────────────────────────────────────────────────')
            lines.append('// ITEMS REQUIRING MANUAL REVIEW')
            lines.append('// ────────────────────────────────────────────────────────────────')
            for item in translator.review_items:
                lines.append(f'// {item}')

        lines.append('')
        return '\n'.join(lines)

    def _assign_dependency_ordering(self, raw_rules):
        rule_list = []
        for rule_obj, text in raw_rules:
            uses, sets = parse_rule(text)
            rule_list.append({'obj': rule_obj, 'uses': uses, 'sets': sets, 'name': rule_obj.name})

        graph = defaultdict(list)
        in_degree = {r['name']: 0 for r in rule_list}

        for r in rule_list:
            for used_field in r['uses']:
                for other in rule_list:
                    if used_field in other['sets'] and other['name'] != r['name']:
                        graph[other['name']].append(r['name'])
                        in_degree[r['name']] += 1
                        break
        ...
        for r in rule_list:
            lvl = level.get(r['name'], cur_level)
            r['obj'].agenda_group = f"phase-{lvl:03d}"

    def _inject_agenda_group(self, rule_text: str, group: str):
        return re.sub(r'    salience \d+', f'    agenda-group "{group}"', rule_text)

    def _build_drl_header(self, source_file, title, translator):
        lines = [
            f'package {self.package_name};',
            '',
            '// ' + '=' * 76,
            f'// Rules converted from OIA document: {source_file}',
            f'// Title: {title}' if title else '',
            f'// Generated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}',
            '// v3.0.0-G — Automatic dependency-aware agenda-group ordering',
            '// Items flagged for review are marked with TODO comments.',
            '// ' + '=' * 76,
            '',
            f'import {self.package_name}.{self.fact_class_name};',
            ''
        ]
        return lines
    ...
    def _generate_rule(self, rule, translator):
        ...
        if hasattr(rule, 'table_row_index') and rule.table_row_index is not None:
            rule_name = f"{rule_name} (row {rule.table_row_index})"
        lines.append(f'rule "{rule_name}"')
        lines.append('    salience 999')
        lines.append('    when')

        if rule.is_otherwise:
            lines.append(f'        $fact : {self.fact_class_name}(')
            # Use == null for otherwise rows (safer for catch-all, fixes FLIP table)
        ...
        if rule.conclusion_type == 'boolean':
            val = 'false' if rule.conclusion_value is False else 'true'
            cap = rule.conclusion_field[0].upper() + rule.conclusion_field[1:]
            lines.append(f'        $fact.set{cap}({val});')
        elif rule.conclusion_type == 'String':
            cap = rule.conclusion_field[0].upper() + rule.conclusion_field[1:]
            lines.append(f'        $fact.set{cap}("{rule.conclusion_value}");')
        elif rule.conclusion_type in ('Integer', 'Double'):
            cap = rule.conclusion_field[0].upper() + rule.conclusion_field[1:]
            lines.append(f'        $fact.set{cap}({rule.conclusion_value});')

        if rule.source_type == 'table' and not rule.is_otherwise:
            undef = rule.conclusion_field + 'Undefined'
            if undef in translator.fields:
                cap = undef[0].upper() + undef[1:]
                lines.append(f'        $fact.set{cap}(false);')
...
# ─────────────────────────────────────────────────────────────────────────────
# convert_document (updated to use v3 generator)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Flask + HTML (your original template with version updated)
# ─────────────────────────────────────────────────────────────────────────────
